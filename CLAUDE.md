...
- **Games (33)**: TicTacToe (minimax), ConnectFour (alpha-beta), RPS, Dots&Boxes (chain-aware AI), Yahtzee, Snake, 2048, Breakout, WhackAMole, Simon, Reaction Timer, Pong, Tap Flight (flappy), Blackjack (chips/betting), HigherLower, VideoPoker (Jacks or Better), Solitaire (Klondike, tap-to-move + undo), Minesweeper, Sudoku (unique-solution generator), LightsOut, MemoryMatch, WordGuess (Wordle-style), Hangman, plus Sims: Sheepdog (boids herding), Traffic Tycoon (intersection sim), Lemonade Stand (economic sim), Outbreak (turn-based SIR grid), plus Watch (ambient, non/minimally interactive): Particle Life, Wealth of Ants (Boltzmann + Gini), Forest Fire (Drossel-Schwabl), Window Lights (real-clock city), Moss Garden (persistent real-time growth), A Small Life (narrated villager, persistent)
